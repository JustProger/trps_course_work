...
@login_required
def lead_comment_delete(request, pk):
    comment = get_object_or_404(Comment, created_by=request.user, pk=pk)
    comment_lead = comment.lead
    comment.delete()

    messages.success(request, 'The lead comment was deleted.')

    return redirect('leads:detail', pk=comment_lead.id)


@login_required
def lead_file_delete(request, pk):
    file = get_object_or_404(LeadFile, created_by=request.user, pk=pk)
    file_lead = file.lead
    ...
    messages.success(request, 'The lead file was deleted.')

    return redirect('leads:detail', pk=file_lead.id)
